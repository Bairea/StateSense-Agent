...
**V0 已实现并通过端到端验证；V0.5 的回放、报表、健康信号和全屏信号已写入代码，真实运行验收尚未全部完成；阶段 3 的影子模型信号链路（任务 3.2 + 3.4 离线部分）与失败样本集（任务 3.1）已落地，影子默认关闭。** 2026-09-24 在当前检出的代码上运行：529 个测试通过，六个回放剧本全部通过（含新增的影子剧本）。判定规则版本已落库（schema v7），影子候选落库（schema v8，`shadow_signals` 表与视图 9），回执前侧改用触发那一轮的证据，效果分析改用同一批干预记录并分层。连续运行五天、跨天阈值判断与真实弹窗回执仍须按阶段 0 计划重新取证；LLM 的增量价值尚无任何证据（影子记录目前全部来自离线替身），本机没有连续运行数据，不能据此判断运行机器此刻的状态。
