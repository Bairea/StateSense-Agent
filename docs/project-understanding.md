...
| 版本 | 内容 | 要验证的问题 | 状态 |
| --- | --- | --- | --- |
| **V0** | Python + Screenpipe API + Rule Engine + 投递 + 双轨回执 + SQLite | 这种提醒有没有价值？ | **已实现**（PR #2） |
| **V0.5** | 回放器 + `report` 观测层 + 常驻健康信号 + 全屏信号 | V0 到底在不在跑，跑出来的数据能不能用？ | **实现完成，验收进行中**（PR #4 规格已提交并修订；代码在本分支；判据 3「跑满 5 天」待达） |
| **V1** | 用真实数据校准阈值与分类；「什么干预最有效」分析 | 什么时候提醒最好、什么干预最有效？ | 待定 |
| **V2** | + LLM | 模糊状态与个性化策略 | 待定 |

> **V0.5 的状态说明（2026-09-18 更新）。** 规格早已不是「待评审」——PR #4 已提交，
> 且规格本身在实施期间被修订过两次（§3 开了一条自动推断的口子，§4.5 新增全屏信号）。
> 代码侧判据 1、2、5 已达成（回放五剧本全过、`--report` 四个问题都能答、测试 154 → 343
> 不退化）；判据 3（挂常驻跑满 5 天）与判据 4（`ratio_min` 的有依据判断）需要真实时间与数据，
> 尚未闭环。逐条证据见 `docs/plans/2026-09-17-v0.5-verification-log.md`。

> **编号已重新基线化（2026-09-16）。** 本表原先定义的 V0 是「Screenpipe + 一个 `pipe.md`」，V0.5 是「Python + Screenpipe API + Rule Engine」，V1 才是「+ feedback + state transitions + SQLite」。
>
> 实际交付的 V0 直接实现了原先的 V0.5（理由见 V0 规格 §17：投递与行为回执是必需项，Pipe 形态承载不了），并顺带实现了原 V1 的 feedback / state transitions / SQLite。因此版本号按事实重排，避免出现「V0.5 实现了自己」的错位。
>
> 原 V0.5 的问题「什么时候提醒最好？」没有被丢弃 —— 它需要真实数据才能回答，因此成为 V1 的问题；V0.5 先负责让它**变得可回答**。
